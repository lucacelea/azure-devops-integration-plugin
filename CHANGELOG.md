...
- **Free-text pull request search in the sidebar**: the pull request view now includes a `Search Pull Requests` action that filters visible PRs by title, author, source branch, or PR ID. Search composes with the existing sidebar filters, and a `Clear Pull Request Search` action appears while a search is active.
